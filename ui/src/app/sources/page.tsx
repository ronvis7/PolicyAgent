    ...
    <div className="h-full overflow-hidden bg-background">
      <header className="flex min-h-16 items-center gap-3 border-b border-border bg-background/95 px-4 py-3">
        ...
        <div className="min-w-0">
          <h1 className="truncate font-serif text-lg font-semibold tracking-tight text-foreground">数据来源</h1>
        ...
        <div className="mx-auto max-w-5xl space-y-6">
          ...
          <section className="rounded-[18px] border border-border bg-card p-5 shadow-[var(--shadow-card)]">
            ...
            {loading ? (
              ...
              <div className="py-10 text-center text-sm text-muted-foreground">暂无已登记的政策来源。</div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2">
          ...
          <section className="rounded-[18px] border border-border bg-card p-5 shadow-[var(--shadow-card)]">
            ...
            {loading ? (
              ...
              <div className="py-10 text-center text-sm text-muted-foreground">暂无已登记的赛事来源。</div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2">
          ...
          <section className="rounded-[18px] border border-border bg-card p-5 shadow-[var(--shadow-card)]">
            ...
            <p className="mb-3 text-xs leading-6 text-muted-foreground">
              资质机会目录由我们依据各级官方认定办法<span className="font-medium text-muted-foreground">结构化整理</span>
            ...
            {loading ? (
              ...
              <div className="py-10 text-center text-sm text-muted-foreground">暂无资质目录数据。</div>
            ) : (
                    ...
                    <ul className="space-y-2">
                      {group.items.map((q) => (
                        ...
                        >
                          ...
                          <dl className="mt-2 grid grid-cols-[64px_1fr] gap-x-3 gap-y-1 text-xs leading-6 text-muted-foreground">
                            {q.issuer && (
                              <>
                                <dt className="text-muted-foreground">发证机关</dt>
                                <dd>{q.issuer}</dd>
                              </>
                            )}
                            {q.policy_basis && (
                              <>
                                <dt className="text-muted-foreground">政策依据</dt>
...
function SourceCard({
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-border bg-card p-4 transition-all hover:-translate-y-0.5 hover:border-brand-200 hover:bg-card hover:shadow-[var(--shadow-card)]">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>
            已收录 <span className="font-semibold text-foreground">{source.policy_count}</span> 条
          </span>
        ...
        {canIngest && onIngest && (
          <Button
            size="sm"
            variant="outline"
            className="h-7 cursor-pointer gap-1 bg-card px-2 text-xs"
            disabled={ingesting}
            onClick={onIngest}
          >
