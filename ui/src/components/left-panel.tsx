...
import {Award, BrainCircuit, Building2, Database, KeyRound, LayoutDashboard, MessageSquareText, Plus, Radar, ScrollText, ShieldCheck, Trophy} from 'lucide-react'
...
const primaryNav = [
  ...
  {href: '/contests', label: '赛事中心', icon: Trophy},
  ...
  return (
    <Sidebar className="border-r border-border bg-sidebar">
      ...
      <SidebarContent className="px-2 pb-2">
        ...
        <Button
          variant="outline"
          className="mb-4 h-10 w-full cursor-pointer justify-center rounded-[10px] border-border bg-card font-semibold shadow-sm hover:bg-card/80"
          onClick={() => router.push('/')}
        >
          <Plus/>
          新建任务
        ...
        <div className="mb-3 px-2 text-[11px] font-semibold tracking-wide text-muted-foreground">企业空间</div>
        {primaryNav.map((item) => {
          ...
          return (
            <Button
              ...
              variant="ghost"
              className={cn(
                'group relative mb-1 h-10 w-full cursor-pointer justify-start rounded-xl px-3 text-foreground transition-colors hover:bg-card',
                active && 'bg-card font-semibold text-brand-700 shadow-[var(--shadow-card)]',
              )}
        ...
        <div className="mt-4 mb-2 px-2 text-[11px] font-semibold tracking-wide text-muted-foreground">最近咨询</div>
        {/* 会话列表 */}
        <SessionList/>
      </SidebarContent>
      {/* 底部：组织切换 + 用户菜单 */}
      <SidebarFooter className="gap-2 px-2 pb-3">
        <div className="rounded-2xl border border-border bg-card p-3 shadow-sm">
          ...
          <div className="mt-1 text-xs text-muted-foreground">角色：{role ?? '-'}</div>
        </div>
        <SidebarMenu>
          {canOpenSettings && (
            <SidebarMenuItem>
