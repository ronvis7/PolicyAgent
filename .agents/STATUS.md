...
- **资质申报机会 ⑥ Phase 1（目录+匹配能力①+接入 ④ Feed）——待确认合并**：把「机会」从单一政策扩为多类，新增资质(qualification)作为 ④ Feed 第二类内容源。**目录即代码、不爬**：`infrastructure/data/qualification_catalog.py` 25 条结构化资质（国家/江苏省/无锡·新吴区/通用认证），每条带 `last_reviewed`+`disclaimer`。**能力①匹配**：纯函数 `qualification_matcher`（地区门槛 + 信号重合 + 前置资质 → 可申报/接近(差N项)，阈值 0.6）+ `QualificationService`。**接入 ④ Feed**：`FeedItem.from_qualification_match`（`policy_id` 复用资质 key、type=qualification）+ `FeedService` 可选 `qualification_service`，抓取/改档案后政策+资质一并物化、统一红点；**零迁移**（`policy_matches` 表 `type` 列既有、`policy_id` 无外键）。API `GET /qualifications` + `/{key}`；UI 资质机会页+Feed 类型徽章与详情分流+左栏入口，详情**强制免责声明+末次核对日期**（风险纪律）。单测 matcher 8+service 4+catalog 6+Feed 2，全量 **102 passed**；tsc/eslint 绿；`alembic heads` 仍 `f7a8b9c0d1e2`（无迁移）。**分支 `feat/qualification-opportunities`（PR #21，CI 全绿，待用户明天确认自行合并）。** 后续：能力②差距分析、③材料指引（Agent 工具）单独迭代。详见 handoff `2026-06-15-qualification-opportunities`。
