...
**私有政策库 + 双轨 Embedding（ADR 003）—— 阶段 A+B 均已合并 main**。**阶段 A**（PR #42，迁移
`b9c0d1e2f3a4` 已落 .222）：租户级 Embedding BYO key。**阶段 B（PR #43）**：① Agent `knowledge_base_search` 去全局公开库（`_resolve_kb_scopes` 删
...
**走查发现并已修**：文件下载/预览报 401（`fileApi.downloadFile` 裸 fetch 未带 `Authorization`，与本主线无关
的存量前端 bug），已注入 Bearer 令牌 + 401 刷新重试（**PR #44**）。
**收尾（PR #45）**：批量收藏（公开政策库勾选→一次收藏多篇入私有政策库）+ 知识库列表真实文件数
（仓储 `count_by_tenant` 单次 GROUP BY，替换随机占位）。
**阶段 A+B + 收尾真机走查已通过**（2026-06-18 用户本机连 .222 实跑：建私有政策库→单篇/批量收藏→indexed→
Agent 命中私有切片且不串公开库→文件下载/预览正常，无问题）。
...
- 跨租户隔离：① 手动对撞探针 `scripts/cross-tenant-probe.ps1`（18 项，连真实栈）；② **自动化 endpoint 隔离测试已进 CI**（PR #46，`api/tests/app/isolation/`，9 项：会话/知识库读删、企业档案不串、**Feed 改状态**、**文件下载**、无/坏令牌→401，均"跨租户→404 + 本租户→成功"双向断言；TestClient + 依赖覆盖 + 内存仓储，不连库）。CI backend job 已从只跑 `tests/app/domain` 扩为跑全部离线测试（忽略需真库的 `test_get_status`），**205 passed**。**局限**：内存仓储测的是端点/服务"是否以 `current_user.tenant_id` 作用域"层；仓储 SQL 层 WHERE 回归需 **DB-in-CI 集成测试**（后续）。**仍未自动化**：成员 `membership_id` 横向越权、租户 LLM key 读取、会话 chat/stop/bind-kb（可按隔离套件模式续加）。详见 handoff `2026-06-18-cross-tenant-isolation-probe` / `-cross-tenant-isolation-ci`。
