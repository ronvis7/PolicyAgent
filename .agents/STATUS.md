...
的存量前端 bug），已注入 Bearer 令牌 + 401 刷新重试（**PR #44，2026-06-18 合并**）。
**阶段 A+B 真机走查仍待做**（连 .222 重建：建私有政策库→收藏政策→indexed→Agent 命中私有切片且不再串
公开库；并双账号验证收藏切片组织间隔离）。
...
- 跨租户隔离已用对撞探针**系统验证通过**（2026-06-18，`scripts/cross-tenant-probe.ps1`，18 项全绿）：会话/知识库读删跨租户→404、企业档案数据不串、成员按 `membership_id` 横向越权→403/404、query 覆盖 `tenant_id` 无效、无令牌→401。**已覆盖**会话/知识库/企业档案/成员/参数注入/未认证；**尚未纳入探针**：Feed 条目改状态、租户 LLM key 读取、文件下载、会话 chat/stop/bind-kb（同源 `current_user.tenant_id` 守卫，风险同源待闭合）。后端多租户**自动化**测试覆盖仍不足（探针是手动对撞、未进 CI）。详见 handoff `2026-06-18-cross-tenant-isolation-probe`。
