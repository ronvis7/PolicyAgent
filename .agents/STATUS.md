...
- `main` 已合入 ②公开政策库（PR #12，含抓取端点）+ ③政策匹配（PR #13，2026-06-15 合并）。
...
1. **④ 工作台 Feed —— 已实现（TDD 完成），待真机迁移 + UI 联调**：在 ③ 即时匹配之上物化 `policy_matches` 表，持久化政策信息流 + 未读红点；`/feed` 取代 `/matches`，左栏入口「工作台」带红点。触发 (a)抓取后+(b)改档案后后台重算当前租户 + 手动「重新匹配」；状态机 unread/read/applied/ignored；**数据模型已预留 `type` 扩展位**（为 ⑥ 资质作第二类机会源）。后端域模型/仓储/服务/ORM/迁移/路由/触发 + 前端 `/feed` 页/左栏红点/`feed.ts` 全落地；删除 `/matches` 页。单测 74 passed（+7），tsc/eslint 绿，迁移 head `f7a8b9c0d1e2`。**分支 `feat/workbench-feed`（PR 待建，迁移未在真机执行）**。详见 handoff `2026-06-15-feed-impl`。
2. **⑥ 资质申报机会 —— 决策固化、种子目录已整理，排在 ④ 之后**：企业方关注资质申请（+比赛）。资质做成 Feed 第二类机会源（opportunity 抽象），数据源用「资质目录」打底不爬（吸取 ①b 教训），三能力（匹配可申报资质 / 条件差距分析 / 材料流程指引）复用 ③ 匹配器 + Agent。首版种子目录（~24 条，国家/省/市区/通用认证）及完整方案见 handoff `2026-06-15-qualification-opportunities`，数值类条件待校对。比赛因走公众号（微信封闭）暂缓，后续开调研小分支配可逆向官网源。
3. 公开库语义检索接入 Agent（KnowledgeBaseTool 纳入 is_public 库）：③刻意未做，留作后续小分支。
4. 报告生成流水线。

## 分支/PR 状态（2026-06-15 收尾）
- `main`：① 企业档案（PR #10）+ ② 公开政策库（PR #12）+ ③ 政策匹配（PR #13，已合）。
