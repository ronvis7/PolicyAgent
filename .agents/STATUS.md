...
- `main` 已合入 ②公开政策库（PR #12，含抓取端点）+ ③政策匹配（PR #13）+ ④工作台 Feed（PR #16，2026-06-15 合并）。
...
1. **⑥ 资质申报机会 —— 决策固化、种子目录已整理，下一块要做**：企业方关注资质申请（+比赛）。资质做成 ④ Feed 第二类机会源（opportunity 抽象，Feed 已预留 `type` 扩展位），数据源用「资质目录」打底不爬（吸取 ①b 教训），三能力（匹配可申报资质 / 条件差距分析 / 材料流程指引）复用 ③ 匹配器 + Agent。首版种子目录（~24 条，国家/省/市区/通用认证）及完整方案见 handoff `2026-06-15-qualification-opportunities`，数值类条件待校对。比赛因走公众号（微信封闭）暂缓，后续开调研小分支配可逆向官网源。
2. 公开库语义检索接入 Agent（KnowledgeBaseTool 纳入 is_public 库）：③刻意未做，留作后续小分支。
3. 报告生成流水线。

## 分支/PR 状态（2026-06-15 收尾）
- `main`：① 企业档案（PR #10）+ ② 公开政策库（PR #12）+ ③ 政策匹配（PR #13）+ ④ 工作台 Feed（PR #16，已合）。
