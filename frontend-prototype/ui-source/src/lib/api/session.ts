...
import { mockSessionApi } from "./mock-session";
import type {
  Session,
  SessionDetail,
  SessionsData,
...
const USE_MOCK_SESSION_API =
  process.env.NEXT_PUBLIC_USE_MOCK_SESSION_API !== "false";
...
export const sessionApi = {
  ...
  getSessions: (): Promise<SessionsData> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.getSessions();
  ...
  createSession: (params?: CreateSessionParams): Promise<Session> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.createSession();
  ...
  streamSessions: (
    ...
    if (USE_MOCK_SESSION_API) return mockSessionApi.streamSessions(onSessions);
  ...
  getSession: (sessionId: string): Promise<Session> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.getSession(sessionId);
  ...
  getSessionDetail: (sessionId: string): Promise<SessionDetail> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.getSessionDetail(sessionId);
  ...
  chat: (
    ...
    if (USE_MOCK_SESSION_API) {
      return mockSessionApi.chat(sessionId, params, onEvent, onError);
    }
  ...
  stopSession: (sessionId: string): Promise<void> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.stopSession();
  ...
  deleteSession: (sessionId: string): Promise<void> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.deleteSession(sessionId);
  ...
  clearUnreadMessageCount: (sessionId: string): Promise<void> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.clearUnreadMessageCount();
    return post<void>(
  ...
  getSessionFiles: (sessionId: string): Promise<SessionFile[]> => {
    if (USE_MOCK_SESSION_API) return mockSessionApi.getSessionFiles();
  ...
  viewFile: (
    ...
    if (USE_MOCK_SESSION_API) return mockSessionApi.viewFile();
  ...
  viewShell: (
    ...
    if (USE_MOCK_SESSION_API) return mockSessionApi.viewShell();
