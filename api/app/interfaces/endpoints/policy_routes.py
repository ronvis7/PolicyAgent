...
@router.get(
    path="/contest-regions",
    response_model=Response[List[str]],
    summary="列出有赛事入库的参赛地区(数据驱动选项)",
    description=(
        "返回赛事来源(item_type=competition)已入库赛事的去重地区，供档案「参赛关注地区」"
        "多选。仅展示真有赛事数据的地区(创客中国等来源一源多地区)。所有登录用户可访问。"
    ),
)
async def list_contest_regions(
        _current_user: CurrentUser = Depends(get_current_user),
        service: PolicyService = Depends(get_policy_service),
) -> Response[List[str]]:
    """列出实际有赛事入库的参赛地区(供档案参赛地区选项)"""
    regions = await service.list_contest_regions()
    return Response.success(data=regions)
